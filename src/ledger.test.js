...
test('members missing from the trip still get a balance instead of NaN',()=>{const trip={members:['A'],expenses:[{payer:'B',cents:50,shares:{A:50}}]};assert.deepEqual(balances(trip),{A:-50,B:50});assert.deepEqual(settlements(trip),[{from:'A',to:'B',cents:50}])});
